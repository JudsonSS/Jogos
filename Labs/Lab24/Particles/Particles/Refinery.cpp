...
void Refinery::Update()
{
	...
	if (window->KeyCtrl('N'))
	{
		...
		if (smokeOn)
		{
			// remove emissores de fumaça
			for (const auto & elem : smoke)
			scene->Delete(elem, STATIC);

			// cria novos emissores de fumaça
			float ang = smokeAng(mt);
			smoke[0] = new Smoke(240,  255, ang, nightOn);
			smoke[1] = new Smoke(380,  312, ang, nightOn);
			smoke[2] = new Smoke(919,  288, ang, nightOn);
			smoke[3] = new Smoke(1041, 293, ang, nightOn);
