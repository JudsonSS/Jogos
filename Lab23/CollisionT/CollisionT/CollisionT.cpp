...
void CollisionT::Update()
{
	...
	if (window->KeyCtrl('R'))
	{
		...
		obj[index]->rotation = 0;
		obj[index]->scale = 1;
		obj[index]->bbox->rotation = 0;
		obj[index]->bbox->scale = 1;

		// polígonos possuem uma bounding box interna
		if (obj[index]->bbox->type == POLYGON_T)
		{
			Poly * pol = static_cast<Poly*>(obj[index]->bbox);
			pol->BBox()->rotation = 0;
			pol->BBox()->scale = 1;
		}

		// objetos mistos possuem sub-objetos
		if (obj[index]->bbox->type == MIXED_T)
		{
			Mixed * mix = static_cast<Mixed*>(obj[index]->bbox);
...
void CollisionT::Draw()
{
	...
	text.str("");
	text << int(window->MouseX()) << " x " << window->MouseY();
